...
def build_prompt(project: Project, design: GeneratedDesign) -> str:
    ...
    return (
        ...
        "}\n\n"
        "Deriving the floor plan from the images (IMPORTANT):\n"
        "- The top-down plan MUST match the arrangement actually shown in the "
        "selected generated design image. Read that image and place each item "
        "where it really sits, not in a generic guessed layout.\n"
        "- Camera-to-plan mapping: the generated photo is taken from one side of "
        "the room. Map left/right in the photo to the x axis (0 = far left wall, "
        "100 = far right wall). Map depth to the y axis: items NEAR the camera "
        "(bottom of the photo) get a LARGER y; items against the FAR wall (top of "
        "the photo, where windows/fireplace usually are) get a SMALL y.\n"
        "- Put each window/door on the wall where it actually appears in the "
        "photo (e.g. windows on the far wall -> y near 0; a door on the right "
        "wall -> x near 100).\n"
        "- Size each box roughly to the item's real footprint relative to the "
        "room (a sofa is wide; a floor lamp is small but still >= 4 wide). A rug "
        "spans under the seating group; let furniture overlap the rug.\n\n"
        "Hard rules:\n"
